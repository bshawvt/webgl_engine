(function() {
	...
	*/
	Graphics.prototype.drawModel = function(model) {
		...
		//gl.drawElements(model.mode, model.count, 
	...
	Graphics.prototype.setClearColor = function(r, g, b, a) {
		a = a ?? 0xff;
		// javascript converts strings to numbers during bitwise operations
		if (typeof r == "string") {
			var offset = 16;
			if (r.length > 8) {
				offset = 8;
	...
	Graphics.prototype.createModel = function(mesh, resource) {
		var metadata = mesh.metadata;
		if (metadata) {
			if (metadata.ovo_version) {
				console.warn("todo: Ovo mesh to model");
				resource.data = mesh;
			}
		}
	};
