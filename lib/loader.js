(function() {
	...
	Loader.prototype.load = function(sources, fnOnDone, fnOnLoad, fnNotFound) {
				...
				if (req.readyState == 4) {
					...
					if (eq.status == 200) {
						...
						try {
							...
							self.add(filename, data);//addResource(filename, data);
						}
						catch (e) {
							console.error(e);
						}
			...
			req.onreadystatechange = function() {
				...
				if (req.readyState == 4) {
					if (req.status == 404)
						return onNext(filename, false);
					if (req.status == 200) {
						self.add(filename, new Uint8Array(req.response));//addResource(filename, new Uint8Array(req.response));
						onNext(filename, true);
					}
				};
			};
		...
		function getImage(url, filename) {
			...
			element.onload = function(event) {
				onNext(filename, true);
				self.add(filename, data);//addResource(filename, element);
			};
			element.src = url;
